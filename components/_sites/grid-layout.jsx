import {Button, Grid, Text, Avatar, Link, Card, Display, Spacer, Divider, Badge} from '@geist-ui/core';
...
export default function GridLayout({data, socialLinks, visibleBlocks, copyHandler}) {
    ...
    return (
        <>
            ...
            <Spacer/>
            <Grid.Container direction="row" justify="space-between" alignItems="center" style={{minWidth: "200px", padding: "1rem", maxWidth: "375px", marginLeft: "auto", marginRight: "auto"}}>
                { visibleBlocks?.map((block, index) => {
                    if(block.type === "card"){
                        return (
                            <CardBlock key={`block-${block.id}-index-${index}`} data={block.data} layout={"grid"} />
                        )
                    }
                    else if(block.type === "link"){
                        return (
                            <LinkBlock key={`block-${block.id}-index-${index}`} layout={"grid"} block={block} site={{
                        ...
                        return (
                            <TwitterCardBlock key={`block-${block.id}-index-${index}`} layout={"grid"} data={block.data}/>
                        )
                    }
                    else if(block.type === "youtube-video"){
                        return (
                            <YouTubeCardBlock key={`block-${block.id}-index-${index}`} layout={"grid"} data={block.data}/>
                        )
                    }
                    else if(block.type === "github-gist"){
                        return (
                            <GithubGistCardBlock key={`block-${block.id}-index-${index}`} layout={"grid"} data={block.data}/>
                        )
                    }
                    else if(block.type === "image"){
                        return (
                            <ImageCardBlock key={`block-${block.id}-index-${index}`} layout={"grid"} data={block.data}/>
                        )
                    }
