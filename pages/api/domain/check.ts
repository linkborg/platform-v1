...
export default async function post(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (subdomain) {
      ...
      let sub_x = sub.toLowerCase();

      // Check if the subdomain is in the disallowSubdomains array
      if (disallowSubdomains.includes(sub_x)) {
        console.error("Not allowed subdomain: ", sub_x)
